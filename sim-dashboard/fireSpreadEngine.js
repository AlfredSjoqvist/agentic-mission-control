...
// ─── Fuel Models (simplified Scott & Burgan) ─────────────────────────────────
// Base spread probability per timestep when conditions are neutral
const FUEL_MODELS = {
  chaparral:    { baseRate: 0.12, intensity: 1.0,  moistureSensitivity: 1.2 },
  grass:        { baseRate: 0.18, intensity: 0.6,  moistureSensitivity: 1.5 },
  timber:       { baseRate: 0.06, intensity: 0.8,  moistureSensitivity: 0.8 },
  urban:        { baseRate: 0.03, intensity: 0.4,  moistureSensitivity: 0.5 },
  rock_bare:    { baseRate: 0.0,  intensity: 0.0,  moistureSensitivity: 0.0 }, // non-combustible
};
...
export class FireSpreadEngine {
  ...
  _spreadProbability(fromRow, fromCol, dirIndex) {
    ...
    // ── Moisture factor ──
    // Lower moisture = higher spread. Dead fuel moisture < 5% is extreme.
    const moisture = this.fuelMoisture[toIdx];
    const moistureFactor = Math.max(0.05, 1.0 - (moisture - 2) * fuel.moistureSensitivity * 0.08);
    P *= moistureFactor;
